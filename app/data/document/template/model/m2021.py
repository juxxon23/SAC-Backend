content_21 = [
    [
        'ACTA No.\n', 
        [
            'NOMBRE DEL COMITÉ O DE LA REUNIÓN:', 
            '', 
            '\n'
        ], 
        [
            [
                'CIUDAD Y FECHA:', 
                '',     
                '\n'
            ], 
            [
                'HORA INICIO:', 
                '', 
                '\n'
            ], 
            [
                'HORA FIN:', 
                '', 
                '\n'
            ]
        ], 
        [
            [
                'LUGAR Y/O ENLACE:', 
                '', 
                '\n'], 
            [
                'DIRECCIÓN GENERAL / REGIONAL / CENTRO', 
                '', 
                'Centro de Comercio y Turismo'
            ]
        ], 
        [
            'AGENDA O PUNTOS PARA DESARROLLAR:', 
            '\n'
        ], 
        [
            'OBJETIVO(S) DE LA REUNIÓN:', 
            '\n'    
        ], 
        'DESARROLLO DE LA REUNIÓN', 
        '\n', 'CONCLUSIONES', 
        '\n', 
        [
            'De acuerdo con La Ley 1581 de 2012, Protección de Datos Personales se debe garantizar la seguridad y protección de los datos personales que se encuentran almacenados en este documento. El Servicio Nacional de Aprendizaje SENA solicita la siguiente clasificación de la información:', 
            '', 
            '', 
            'La información de este documento se debe clasificar como:', 
            '', 
            '', 
            'PÚBLICA  PRIVADO  SEMIPRIVADO  SENSIBLE  ', 
            '', 
            '', 
            'Nota: Antes de contestar esta información por favor remitirse al instructivo'
        ], 
        'COMPROMISOS', 
        [
            'ACTIVIDAD', 
            'RESPONSABLE', 
            'FECHA'
        ], 
        [
            '\n', 
            '\n', 
            '\n'
        ], 
        [
            'ASISTENTES: (Incorporar registro de asistencia)', 
            '', 
            'Nota:', 
            '', 
            ''
        ]
    ], 
    [
        'REGISTRO DE ASISTENCIA Y APROBACIÓN DEL ACTA No- 3 DEL DÍA 09 DEL MES DE FEBRERO DEL AÑO 2021', 
        [
            'OBJETIVO(S)', 
            '\n'
        ], 
        [
            'No.', 
            'Autorizo el tratamiento de la Información SI / NO. Si marca No por favorno diligencie el formato.', 
            'NOMBRES Y APELLIDOS', 
            'No. DOCUMENTO', 
            'PLANTA', 
            'CONTRATISTA', 
            'OTRO\n¿CUAL?', 
            'DEPENDENCIA/EMPRESA', 
            'CORREO ELECTRÓNICO', 
            'TELÉFONO/EXT.SENA', 
            'AUTORIZA GRABACIÓN', 
            'FIRMA O PARTICIPACIÓN VIRTUAL'
        ], 
        [
            '1', 
            '\n', 
            '\n', 
            '\n', 
            '\n', 
            '\n', 
            '\n', 
            '\n', 
            '\n', 
            '\n', 
            '', 
            '\n'
        ]
    ]
]