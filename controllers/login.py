...
from validators.user_u import LoginUser
    ...
    def post(self):
        ...
        if document_cp != None:
            if bcrypt.checkpw(pass_ex, document_cp.password_u.encode('utf-8')):
                encoded_jwt = jwt.encode({'exp': datetime.datetime.utcnow() + datetime.timedelta(seconds=300), 'nombre': document_cp.name_u}, KEY_TOKEN_AUTH,algorithm='HS256')
